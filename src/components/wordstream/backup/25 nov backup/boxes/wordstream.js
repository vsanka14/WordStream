...
export default function wordStream(data) {
    ...
    let size = [1200, 800],
        ...
        maxFrequencyOfYear = null,
        maxSud, minSud, maxFreq;

    wordStream.buildScales = function() {
        fontScale = generateFontSizeScale(data);
    ...
    function generateFontSizeScale() {
        ...
        return d3.scaleLinear().domain([min, max]).range([minFontSize, maxFontSize]);
    }

    // function generateFontSizeScale(data) {
    //     let minSudden = Math.pow(10, 1000);;
    //     let maxSudden = 0;
    //     data.map((box)=>{
    //        topics.map(topic=>{
    //             let maxS = d3.max(box.words[topic], d=>d.sudden);
    //             let minS = d3.min(box.words[topic], d=>d.sudden);
    ...
    function buildFrequencyScale(data){
        ...
        var sumMaxFrequencyOfYear = 0;
        d3.map(totalFrequencies, function(d){
            var total = 0;
            topics.forEach(key=>{
                total+=d[key];
            })
            if(total > sumMaxFrequencyOfYear) sumMaxFrequencyOfYear = total;
        });
        maxFrequencyOfYear = sumMaxFrequencyOfYear;
        frequencyScale.domain([-sumMaxFrequencyOfYear, sumMaxFrequencyOfYear]).range([0, size[1]]).nice();
    }

    function calculateTotalFrequenciesABox(data){
        var totalFrequenciesABox = [];
        for(var d=0; d<data.length; d++)
        {
            var object = {}
            object['date'] = data[d].date;
            let words = data[d].words;
            Object.keys(data[d].words).forEach(topic=>{
                let sum = 0;
                Object.entries(words[topic]).forEach(([key, value])=>{
                    sum+=parseInt(value.frequency);
                });
                object[topic] = sum;
            });
            totalFrequenciesABox.push(object);
        }
        ...
        topics.forEach((topic, i)=>{
            ...
            stackedLayers[i].forEach(layer=>{
                innerBoxes[topic].push({
                    x: layer.data.date,
                    y: frequencyScale(layer[0]),
                    width: boxWidth,
                    height: frequencyScale(layer[1]) - frequencyScale(layer[0])
                });
            });
        });
        boxes = {
    ...
    function buildSvg(boxes, topic){
        ...
        var area1 = d3.area()
            ...
            .y0(0)
            .y1(function(d){return frequencyScale(d[0]); });

        var area2 = d3.area()
            .curve(d3.curveLinear)
            .x(function(d, i){return (i)*100 })
            .y0(function(d){return (frequencyScale(d[1])); })
            .y1(height);
    ...
    wordStream.frequencyScale = () => frequencyScale;
    // wordStream.minSud = () => minSud;
    // wordStream.maxSud = () => maxSud;
    wordStream.maxFrequencyOfYear = () => maxFrequencyOfYear;
    return wordStream;
}   
