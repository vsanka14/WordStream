...
var totalData;
export async function loadBlogPostData(fileName, categories, top){
    var topics = [];
    let rawData = await d3.tsv(fileName);
    var inputFormat =  d3.timeParse('%Y-%m-%dT%H:%M:%S');
    var outputFormat = d3.timeFormat('%b %Y');
    topics = categories;

    rawData = rawData.filter(function(d){
        var time = Date.parse(d.time);
        var startDate =  inputFormat('2012-12-01T00:00:00');
        var endDate = inputFormat('2014-01-01T00:00:00');
        //2011 for CrooksAndLiars
        if(fileName.indexOf("Liars")>=0){
            startDate = inputFormat('2009-12-01T00:00:00');
            endDate = inputFormat('2011-01-01T00:00:00');
        }
        else if(fileName.indexOf("WikiNews")>=0){
            startDate = inputFormat('2013-12-01T00:00:00');
            endDate = inputFormat('2016-01-01T00:00:00');
        }
        else if(fileName.indexOf("Huffington")>=0){
            startDate = inputFormat('2011-12-01T00:00:00');
            endDate = inputFormat('2013-01-01T00:00:00');
        }
...
export async function loadAuthorData(fileName, categories, top){
